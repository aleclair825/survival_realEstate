...
# Create Cox-Snell residuals
# CS_w = -log( 1 - pweibull(house$tom, 1/0.533, exp(4.4549 + 0.246*house$"crimeCat2(250,2e+03]" + 0.1379*) ) )
CS_ln = -log( 1 - plnorm(house$tom, 4.3010 + -0.0313*house$schdist, 0.623) ) 
...
```{r}
me = survreg( Surv(tom) ~ schdist + ncrime + listedatdate + listedatpr , dist = "exponential" , data = house )
mw = survreg( Surv(tom) ~ schdist + ncrime + listedatdate + listedatpr , dist = "weibull" , data = house )
mln = survreg( Surv(tom) ~ schdist + ncrime + listedatdate + listedatpr , dist = "lognormal" , data = house )
