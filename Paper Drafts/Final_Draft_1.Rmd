...
ggsurvplot(KM1, data=house, legend.labs=c("price = 63,000 to 180,000", "180,000 to 275,000", "275,000 to 1,300,000"), xlim=c(0,250)) +
...
ggsurvplot(KM2, data=house, legend.labs=c("Minneapolis", "St. Paul"), xlim=c(0,250)) +
...
Now we must decide which model is better. To do this, we will first plot the Cox-Snell residuals for the Weibull and log-normal models and assess which residual plot fits a line with intercept 0 and slope 1 better. We will then compare the AIC calculations for each model. Utilizing both a graphical and analytical approach to assessing model fit will help ensure thorough vetting of models. 
...
# Make appropriate graph using CoxSnell function
CoxSnell( CS_w , house$status ,  main="Hello" )
CoxSnell( CS_ln , house$status, main = "" )
```
...
log-normal (3240.594) performs better in the AIC calculation than Weibull (3241.18). 
...
#### Crime Category 2 + Listed at Price + School Distance
#### With Interaction Term (if we need this vs. strata !!!)
...
We now look at the categorical listed price variable. 

Kaplan-Meier c-log-log plot also has very narrow gap between the first and second listed price groups, but a clearly unproportional gap in the lowest and the highest listed price groups. The latter trend point to a strong evidence for the violoation of the PH assumption.
...
As seen in the Kaplain-Meier c-log-log plot, the Schoenfeld residual plots illustrate that the PH assumption holds in the first two listed price groups, while the opposite is true for the first and last listed price groups. More specifically, the second group compared to the first group largely maintains the PH assumption throughout except at the very tip of the time frame. On the other hand, the highest listed price group has visibly increasing (from large negative to positive) residuals, indicating a clear violation of the PH assumption. The residual output agrees in that the highest listed price group has a non-constant positive HR with rho = 0.18 and a highly significant p-value. In other words, the hazard ratio is being underestimated as the residuals are increasing. The second group, on the other hand has rho close to 0 with insignificant p-value, indicating the validity of the PH assumption. 
