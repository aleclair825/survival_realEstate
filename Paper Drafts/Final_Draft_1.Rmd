...
Mostly, it's negative but thiseffect is comining in the first two months. 
If the houses are not selling by the two month peirod, there should be a reason why they are not selling. 
We're more interested in the effects in the first two months than we are later.
...
strata = if can't assume the PH, then use strata. (Victor)
...
Talk about coxph for univariate here. 

We considered that some of these variables might also behave differently in the presence of each other, so we created a multivariate model.

Talk about coxph for multivariate here. 

Overall coxph assumptions for each variable here. 
...
Weibull and log-normal output results:
