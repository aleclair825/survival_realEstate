...
Log-normal model had most of the coefficients in the right direction with far worse significance, and the Cox PH model agrees with the Weibull model. 
...
for (pr in 11 : 14) {
  a = exp(1.666+-0.121*pr)
  b = exp(-1.984+0.179*pr)
  c = exp(-2.118+0.186*pr)
  print(lst(c(a,b,c)))
  
}

(min(log(house$listedatpr)))
```
...
exp(5.3589-0.4244*12)
exp(5.3589-0.4244*13)
exp(5.3589-0.4244*14)
...
mcox = coxph(Surv(tom) ~ schdist + crimeCat2 + strata(listedatprCat) , data = house )
#present two models and say that they don't agree completely
```



```{r echo=FALSE, include=FALSE}
mw = survreg( Surv(tom) ~ schdist + crimeCat2 + log(listedatpr) +  schdist:log(listedatpr), dist = "weibull" , data = house )
mln = survreg( Surv(tom) ~ schdist + crimeCat2 + log(listedatpr) + crimeCat2:log(listedatpr) + schdist:log(listedatpr), dist = "lognormal" , data = house )
mcox = coxph(Surv(tom) ~ schdist + crimeCat2 + log(listedatpr) + crimeCat2:log(listedatpr) + schdist:log(listedatpr), data = house )
